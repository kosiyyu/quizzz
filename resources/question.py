...
parser.add_argument('points_per_correct_answer', type=float, required=True)
        ...
        try:
            ...
            if not all(key in args for key in ('text', 'points_per_correct_answer', 'quiz_id')):
                return {
                           ...
                           'Content-Type': 'application/json'}
            question = Question(text=args['text'], points_per_correct_answer=args['points_per_correct_answer'],
                                quiz_id=args['quiz_id'])
            answers_args = args.pop('answers', [])
